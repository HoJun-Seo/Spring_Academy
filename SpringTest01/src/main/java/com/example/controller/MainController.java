...
public class MainController {
	...
	public ModelAndView modelAndView() {
		...
		// 자료전달 형식 : 맵키.클래스 객체이름.클래스 멤버변수
		return new ModelAndView("test/modelAndview.do", "map", map);
	}
	...
	// url 역할 데이터(JSON) 전달 형식은 변환
	@RequestMapping("ajax_background.do")
	public @ResponseBody ProductDTO ajax_backgournd() {
		ProductDTO dto = new ProductDTO("냉장고", 500000);
		
		//JSON 형식의 자료 전송 : url 형식이 아님
		return dto;
	}
